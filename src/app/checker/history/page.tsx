...
export default async function CheckerHistoryPage(
  ...
  const [{ documents, totalCount, totalPages }, sitesData] = await Promise.all([
    getCheckerDocuments({ siteId, status: 'APPROVED', search, page, pageSize }),
    getMyAssignedSites()
  ])
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Approved History</h1>
          <p className="text-muted-foreground">Review previously approved bills.</p>
        </div>
      </div>
